...
def convert(input_grid):
    ...
    for row in range(0, len(input_grid), 4):
        for pos in range(0, len(input_grid[0]), 3):
            one_number = (
                input_grid[0 + row][pos : pos + 3],
                input_grid[1 + row][pos : pos + 3],
                input_grid[2 + row][pos : pos + 3],
                input_grid[3 + row][pos : pos + 3],
            )
            numbers.append(one_number)
        if len(input_grid) > 4 and row != len(input_grid)-4:    
            numbers.append(",")
